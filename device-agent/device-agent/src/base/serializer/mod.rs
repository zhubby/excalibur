...
use crate::device_agent_config::{Compression, StreamConfig};
...
impl<C: MqttClient> Serializer<C> {
    /// Construct the device_agent Serializer with the necessary configuration details, a receiver handle to accept data payloads from,
    ...
    fn initialize_storages(&mut self) {
        for stream_config in self.config.streams.values() {
            self.sorted_storages.insert(
                Arc::new(stream_config.clone()),
                (self.create_storage_for_stream(stream_config), None, 0),
            );
        }
    }
    ...
    fn create_storage_for_stream(&self, config: &StreamConfig) -> StorageEnum {
        if config.persistence.max_file_count == 0 {
            ...
            match storage::DirectoryStorage::new(
                ...
                Err(e) => {
                    log::error!(
                        "Failed to initialize disk backed storage for {} : {e}, falling back to in memory persistence",
                        config.name
                    );
                    StorageEnum::InMemory(storage::InMemoryStorage::new(
                        config.name.as_str(),
    ...
    fn fetch_next_packet_from_storage(&mut self) -> Option<(Publish, Arc<StreamConfig>)> {
        if let Some((sk, (_, live_data, live_data_version))) = self
            .sorted_storages
            .iter_mut()
            ...
            match storage.read_packet() {
                Ok(packet) => {
                    if packet.topic.starts_with(&self.topic_prefix) {
                        ...
                        log::warn!("found data for the wrong project or device in persistence");
                        continue;
                    }
                }
        ...
        if let Some(publish) = packet_to_write {
            match storage.write_packet(publish) {
                ...
                Err(storage::StorageWriteError::FileSystemError(e)) => {
                    ...
                    let (old_storage, old_last_value, old_live_data_version) =
                        self.sorted_storages.remove(&sk).unwrap();
                    self.sorted_storages.insert(
                        sk.clone(),
                        (old_storage.to_in_memory(), old_last_value, old_live_data_version),
                    );
                }
                Err(storage::StorageWriteError::InvalidPacket(e)) => {
                    log::error!(
                        "Found invalid packet when writing to storage for stream({}): {e}",
                        sk.name
                    );
                }
            };
        ...
        for (_, (storage, live_data, _)) in self.sorted_storages.iter_mut() {
            if let Some(publish) = live_data.take() {
                if let Err(e) = storage.write_packet(publish) {
                    log::error!(
                        "Couldn't write live data to storage while flushing for stream({}) : {e:?}",
                        storage.name()
                    );
                }
            }
    ...
    async fn slow(&mut self, publish_in_transit: Publish, stream: Arc<StreamConfig>) -> Status {
        ...
        // Reactlabs setup processes logs generated by device_agent
        log::info!("Switching to slow eventloop mode!!");
        self.metrics.mode = "slow";

        let publish = send_publish(
            self.client.clone(),
            publish_in_transit.topic.clone(),
            publish_in_transit.payload.clone(),
        );
        tokio::pin!(publish);
        ...
        loop {
            select! {
                ...
                // Transition into crash mode when device_agent is shutting down
    ...
    async fn catchup(&mut self) -> Status {
        ...
        let (mut publish_in_transit, mut last_publish_stream) =
            if let Some(publish) = self.fetch_next_packet_from_storage() {
                publish
            } else {
    ...
    /// Starts operation of the device_agent serializer, which can transition between the modes mentioned earlier.
...
pub fn construct_publish(
    ...
    log::trace!(
        "Data received on stream: {stream_name}; message count = {point_count}; batching latency = {batch_latency}"
    );
    ...
    let mut payload = if topic.ends_with("/shadow") {
        data.buffer
            .last()
            .map(|payload| serde_json::to_vec(&payload.payload).unwrap())
            .unwrap_or_else(|| b"{}".to_vec())
    } else if topic.ends_with("/commands/status") {
        let statuses: Vec<_> = data.buffer.iter().map(|payload| payload.payload.clone()).collect();
        serde_json::to_vec(&statuses).unwrap()
    } else {
        data.serialize()
    };
...
pub mod tests {
    ...
    use crate::base::bridge::stream::MessageBuffer;
    use crate::device_agent_config::Persistence;
    use crate::device_agent_config::StreamConfig;
    use crate::{
        device_agent_config::MqttConfig,
        hashmap,
    ...
    pub fn defaults(
        ...
        (
            Serializer::new(config, String::new(), data_rx, client, metrics_tx, ctrl_rx),
            data_tx,
            net_rx,
            ctrl_tx,
        )
    ...
    async fn directory_to_in_memory_queue_behavior() {
        ...
        if let Request::Publish(publish) = net_rx.recv_async().await.unwrap() {
            let payloads =
                serde_json::from_slice::<Vec<SerializedPayload>>(publish.payload.as_ref()).unwrap();
        ...
        for i in 1..147 {
            if let Request::Publish(publish) = net_rx.recv_async().await.unwrap() {
                let payloads =
                    serde_json::from_slice::<Vec<SerializedPayload>>(publish.payload.as_ref())
                        .unwrap();
                assert_eq!(payloads.len(), 1);
                let payload = payloads.first().unwrap();
    ...
    #[test]
    fn telemetry_streams_publish_excalibur_json_arrays() {
        let sk = Arc::new(create_test_stream_config());
        let publish = construct_publish(Box::new(create_test_buffer(42, sk)), &mut HashMap::new());

        assert_eq!(publish.topic, "v1/p/demo/d/1/telemetry/test_stream");
        let payloads =
            serde_json::from_slice::<Vec<SerializedPayload>>(publish.payload.as_ref()).unwrap();
        assert_eq!(payloads.len(), 1);
        assert_eq!(payloads[0].sequence, 42);
        assert_eq!(payloads[0].payload, serde_json::json!({ "msg": "Hello, World!" }));
    }

    #[test]
    fn shadow_topic_publishes_latest_object() {
        let sk = Arc::new(StreamConfig {
            name: "device_shadow".to_string(),
            topic: "v1/p/demo/d/1/shadow".to_string(),
            batch_size: 2,
            ..Default::default()
        });
        let mut buffer = MessageBuffer::new(Arc::new("device_shadow".to_owned()), sk);
        buffer.buffer.push(Payload {
            stream: "device_shadow".to_string(),
            sequence: 1,
            timestamp: 0,
            payload: serde_json::json!({ "state": "old" }),
        });
        buffer.buffer.push(Payload {
            stream: "device_shadow".to_string(),
            sequence: 2,
        ...
        let publish = construct_publish(Box::new(buffer), &mut HashMap::new());

        assert_eq!(publish.topic, "v1/p/demo/d/1/shadow");
        let payload = serde_json::from_slice::<Value>(publish.payload.as_ref()).unwrap();
        assert_eq!(payload, serde_json::json!({ "state": "new" }));
    }

    #[test]
    fn command_status_topic_publishes_status_array() {
        let sk = Arc::new(StreamConfig {
            name: "action_status".to_string(),
            topic: "v1/p/demo/d/1/commands/status".to_string(),
            batch_size: 2,
            ..Default::default()
        });
        let mut buffer = MessageBuffer::new(Arc::new("action_status".to_owned()), sk);
        buffer.buffer.push(Payload {
            stream: "action_status".to_string(),
            sequence: 1,
            timestamp: 0,
            payload: serde_json::json!({
                "action_id": "9bbf7b9e-8a03-4a66-b178-f4c36a11df65",
                "state": "Running",
                "progress": 40,
                "errors": []
            }),
        ...
        assert_eq!(publish.topic, "v1/p/demo/d/1/commands/status");
        let payload = serde_json::from_slice::<Value>(publish.payload.as_ref()).unwrap();
        assert_eq!(
            payload,
            serde_json::json!([{
    ...
    fn create_test_stream_config() -> StreamConfig {
        StreamConfig {
            ...
            topic: "v1/p/demo/d/1/telemetry/test_stream".to_string(),
            batch_size: 2,
            flush_period: Duration::from_secs(1),
            compression: Compression::Disabled,
            persistence: Persistence { max_file_size: 10800, max_file_count: 10 },
            priority: 0,
        }
    }
    ...
    fn _get_test_buffer_size() {
        let sk = Arc::new(StreamConfig {
            ...
            flush_period: Duration::from_secs(1),
            compression: Compression::Disabled,
            persistence: Persistence { max_file_size: 100000, max_file_count: 100000 },
            priority: 0,
        });
    ...
    async fn stream_queue_behavior(
        max_file_size: usize,
        ...
        let sk = Arc::new(StreamConfig {
            ...
            flush_period: Duration::from_secs(1),
            compression: Compression::Disabled,
            persistence: Persistence { max_file_size, max_file_count },
            priority: 0,
        });
        let config = Config {
            persistence_path: temp_dir.path().to_owned(),
    ...
    async fn shutdown_and_reload() {
        ...
        let temp_dir = PathBuf::from("/tmp/device_agent/persistence");
    ...
    async fn catchup_to_crash_with_persistence() {
        ...
        let sk = Arc::new(StreamConfig { topic: "test/topic".to_string(), ..Default::default() });
    ...
    async fn preferential_send_on_network() {
        ...
        let config = Arc::new(config);
        let config_stream_one = Arc::new(StreamConfig {
            topic: "topic/one".to_string(),
            priority: 1,
            ..Default::default()
        });
        let config_stream_two = Arc::new(StreamConfig {
        ...
        serializer
            .write_publish_to_storage(config_stream_one.clone(), create_publish("topic/one", 0));
        serializer
            .write_publish_to_storage(config_stream_one.clone(), create_publish("topic/one", 1));
        serializer.write_publish_to_storage(
            config_stream_three.clone(),
            create_publish("topic/three", 0),
        );
        serializer.write_publish_to_storage(
            config_stream_three.clone(),
            create_publish("topic/three", 1),
        );
        serializer
            .write_publish_to_storage(config_stream_two.clone(), create_publish("topic/two", 0));
        serializer
            .write_publish_to_storage(config_stream_two.clone(), create_publish("topic/two", 1));
        ...
        for _ in 0..2 {
            match mqtt_rx.recv().unwrap() {
                Request::Publish(p) => {
                    ...
                    let payload =
                        serde_json::from_slice::<[SerializedPayload; 1]>(p.payload.as_ref())
                            .unwrap();
                    sequences.push(payload[0].sequence);
                }
                _ => {
                    panic!("boo");
                }
