...
impl EventsPusher {
    pub fn new(
        ...
        Self { pubacks, publisher, project_id, device_id, db_path, reserved_pkids }
    }
    ...
    pub async fn start(self) {
        ...
        loop {
            match &mut state {
                Init => {
                    let result =
                        conn.lock().unwrap().query_row(FETCH_ONE_EVENT, (), EventOrm::create);
                    match result {
                        Ok(event_o) => {
                            match self
                                .generate_publish(&event_o, self.reserved_pkids[current_pkid_idx])
                            {
                                Ok(event) => {
                                    ...
                                    state = SendingEvent(
                                        event_o.id,
                                        Box::pin(
                                            SendOnce(self.publisher.request_tx.clone())
                                                .send_async(event),
                                        ),
                                    );
                                }
                                Err(e) => {
                                    ...
                                    if let Err(e) =
                                        conn.lock().unwrap().execute(POP_EVENT, (event_o.id,))
                                    {
                                        log::error!(
                                            "unexpected error: couldn't pop event from queue: {e}"
                                        );
                                        return;
                                    }
                                }
                ...
                SendingEvent(event_id, fut) => match fut.await {
                    Ok(_) => {
                        ...
                        match tokio::time::timeout_at(end, self.pubacks.recv_async()).await {
                            Ok(Ok(pkid)) => {
                                if pkid == self.reserved_pkids[current_pkid_idx] {
                                    ...
                                    if let Err(e) =
                                        conn.lock().unwrap().execute(POP_EVENT, (*event_id,))
                                    {
                                        log::error!(
                                            "unexpected error: couldn't pop event from queue: {e}"
                                        );
    ...
    fn generate_publish(&self, event: &EventOrm, pkid: u16) -> anyhow::Result<Request> {
        ...
        let mut result = Publish::new(
            format!("v1/p/{}/d/{}/telemetry/{}", self.project_id, self.device_id, payload.stream),
            QoS::AtLeastOnce,
...
fn do_initialization(conn: &Mutex<Connection>) -> anyhow::Result<()> {
    conn.lock().unwrap().execute_batch(CREATE_EVENTS_TABLE)?;

    let count =
        conn.lock().unwrap().query_row(FETCH_EVENTS_COUNT, (), |row| row.get::<_, u64>(0))?;
    if count != 0 {
...
impl EventOrm {
    pub fn create(row: &Row) -> rusqlite::Result<Self> {
        Ok(Self { id: row.get(0)?, payload: row.get(1)? })
    }
}
