...
use crate::base::serializer::storage::{PersistenceError, PersistenceFile};
...
impl Mqtt {
            ...
            match Packet::read(&mut buf, max_packet_size) {
                Ok(Packet::Publish(publish)) => {
                    if publish.topic.starts_with(&self.topic_prefix) {
                        ...
                        warn!("inflight file has data for the wrong project or device");
                    }
                }
    ...
    pub async fn start(mut self) {
        ...
        if self.config.console.enable_events {
            ...
            // the second pkid won't be sent until device_agent has received an acknowledgement for the first pkid
            // and the first pkid won't be sent again until device_agent receives an acknowledgement for the second pkid
        ...
        // TODO: when device_agent uses last-wills to handle unexpected disconnections, try to disconnect from
...
fn mqttoptions(config: &Config, device_config: &DeviceConfig) -> MqttOptions {
    ...
    if let Some(auth) = device_config.authentication.clone() {
        ...
        let device_private_key = auth
            .device_private_key
            .or_else(|| {
                auth.device_private_key_path.and_then(|path| match std::fs::read_to_string(&path) {
                    Ok(key) => Some(key),
                    Err(error) => {
                        error!(
