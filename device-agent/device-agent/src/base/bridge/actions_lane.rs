...
use crate::device_agent_config::{ActionRoute, Config};
...
pub struct ActionsBridge {
    ...
    /// changing the device_agent config format by removing the redirection clause
    ...
    /// used if device_agent is used as a library
...
impl ActionsBridge {
    pub fn new(
        ...
        Self {
            status_tx,
            status_rx,
            actions_routing_cache: Self::load_actions_routing_cache(
                config.persistence_path.as_path(),
            ),
            config,
            actions_rx,
            data_tx,
    ...
    fn load_actions_routing_cache(persistence: &Path) -> LimitedArrayMap<String, String> {
        ...
        let mut result = std::fs::read(&save_file)
            ...
            if let Err(e) = std::fs::remove_file(&save_file) {
                warn!("Couldn't remove a file in persistence directory: {e}. Does the device_agent process have right permissions?");
            }
    ...
    async fn persist_actions_routing_cache(&self) {
        ...
        if let Err(e) = std::fs::write(
            ...
            log::error!(
                "Couldn't write to a file in persistence directory: {e}. Does the device_agent process have right permissions?"
            );
        }
    }
        ...
        match self.action_routes.get(route_id) {
            Some(route) => {
                if let Err(e) = route.try_send(action.clone()) {
                    ...
                    self.forward_action_response(ActionResponse::failure(
                        action.action_id.as_str(),
                        format!("Could not forward action to collector: {e}"),
                    ))
                    .await;
                } else {
                    if let Some((action_id, app_name)) = self
                        .actions_routing_cache
                        .set(action.action_id.to_owned(), route_id.to_owned())
                    {
                        log::info!(
                            "Dropping routing info about action_id: {action_id} executed on app: {app_name}"
                        );
                    }
                ...
                match &self.actions_callback {
                    ...
                    None => {
                        self.forward_action_response(ActionResponse::failure(
                            action.action_id.as_str(),
                            format!(
                                "DeviceAgent isn't configured to handle actions of type {route_id}"
                            ),
                        ))
                        .await;
                    }
    ...
    async fn forward_action_response(&mut self, response: ActionResponse) {
        ...
        if let Some(mut redirected_action) = response.done_response {
            if let Some(target_action) =
                self.action_redirections.get(redirected_action.name.as_str())
            {
