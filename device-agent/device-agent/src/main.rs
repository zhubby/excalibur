...
fn main() -> Result<(), Error> {
    ...
    let device_json =
        read_to_string(commandline.auth.as_path()).context("couldn't read auth json file")?;
    let config_toml = match commandline.config.as_ref() {
        None => String::new(),
        Some(p) => read_to_string(p).context("couldn't read config toml file")?,
    };
    ...
    if controller.end_rx.recv().is_err() {
        log::error!("device_agent stopped without sending to end_tx");
    }
...
#[structopt(name = "device_agent", about = "collect, batch, compress, publish")]
...
pub fn initialize_logging(log_level: u8, log_modules: Vec<String>) {
    ...
    let levels = match log_modules.into_iter().reduce(|e, acc| format!("{e}={level},{acc}")) {
