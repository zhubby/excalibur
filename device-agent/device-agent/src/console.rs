...
use axum::{
    Router,
    extract::State,
    http::{StatusCode, response::Builder},
...
pub async fn start(
    ...
    info!("Starting device_agent console server: {address}");

    let events_db_conn = events_db_path.and_then(|path| {
        Connection::open(path)
            .map_err(|e| {
                log::error!("couldn't connect to events database: {e}");
                e
            })
            .and_then(|conn| match conn.execute_batch(CREATE_EVENTS_TABLE) {
                Ok(_) => Ok(conn),
                Err(e) => {
...
// Pushes device_agent status as JSON text
...
async fn save_events(
    ...
    axum::Json(payload): axum::Json<PayloadWithStream>,
) -> (StatusCode, axum::Json<Value>) {
    match state.events_db_conn {
        None => (
            StatusCode::BAD_REQUEST,
            axum::Json(json!({ "error": "events feature is disabled in device_agent config" })),
        ),
        Some(conn) => {
            let payload = serde_json::to_string(&payload).unwrap();
            match conn.lock().unwrap().execute(INSERT_EVENT, (payload,)) {
                Ok(_) => (StatusCode::OK, axum::Json(json!({ "result": "ok" }))),
                Err(e) => {
                    log::error!("couldn't save event to local storage: {e:?}");
                    (
                        StatusCode::INTERNAL_SERVER_ERROR,
                        axum::Json(json!({ "error": "couldn't save event to local storage" })),
                    )
                }
            }
        }
