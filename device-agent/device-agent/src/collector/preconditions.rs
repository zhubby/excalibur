...
use crate::{Action, ActionResponse, Config, base::bridge::BridgeTx};
...
impl PreconditionChecker {
    ...
    pub async fn start(self) {
        while let Ok(action) = self.actions_rx.recv() {
            let preconditions: Preconditions = match action.payload_as::<Preconditions>() {
                Ok(c) => c,
                Err(e) => {
            ...
            let response =
                ActionResponse::done(&action.action_id, "Checked OK", Some(action.clone()));
            self.bridge_tx.send_action_response(response).await;
        }
    }
