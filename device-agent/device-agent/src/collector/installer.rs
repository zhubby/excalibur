...
use crate::{Action, ActionResponse, base::bridge::BridgeTx, device_agent_config::InstallerConfig};
...
impl OTAInstaller {
    ...
    fn extractor(&self, action: &Action) -> Result<(), Error> {
        let info: DownloadFile = action.payload_as::<DownloadFile>()?;
    ...
    async fn installer(&self, action: &Action) -> Result<(), Error> {
        ...
        cmd.arg(&action.action_id).arg(self.config.device_agent_port.to_string());
        cmd.spawn()?;
