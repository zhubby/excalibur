...
use crate::device_agent_config::{AppConfig, Compression, MAX_BATCH_SIZE, StreamConfig};
use base::CtrlTx;
pub use base::actions::{Action, ActionResponse};
use base::bridge::{Bridge, Payload, StreamMetrics, stream::Stream};
...
pub struct DeviceAgentController {
    pub data_tx: Sender<Payload>,
    /// can be used to trigger shutdown
    /// device_agent will stop everything it's doing and send a message on end_rx when everything has stopped
    pub shutdown: CtrlTx,
    pub end_rx: Receiver<()>,
}
...
impl DeviceAgent {
    ...
    pub fn spawn(
        ...
        let topic_prefix =
            format!("v1/p/{}/d/{}", device_config.project_id, device_config.device_id);
    ...
    pub fn spawn_builtins(&mut self, bridge: &mut Bridge) -> Result<(), Error> {
        ...
        if self.config.enable_remote_shell {
            let route = ActionRoute { name: "launch_shell".to_owned() };
        ...
        if self.config.device_shadow.enabled {
            let device_shadow =
                DeviceShadow::new(self.config.device_shadow.clone(), bridge_tx.clone());
        ...
        if let Some(config) = self.config.logging.clone() {
            let route = ActionRoute { name: "journalctl_config".to_string() };
            let actions_rx = bridge.register_action_route(route)?;
...
pub fn entrypoint(
    ...
    let mut device_agent = DeviceAgent::new(config.clone(), device_config.clone())?;
    let mut bridge = device_agent.configure_bridge(actions_callback);
    device_agent.spawn_builtins(&mut bridge)?;
    ...
    let ctrl_tx = device_agent.spawn(
        &device_config,
        bridge,
        downloader_disable.clone(),
        network_up.clone(),
    )?;
    ...
    if config.console.enabled {
        ...
        spawn_named_thread("DeviceAgent Console", move || {
            console::start(
                port,
                downloader_disable,
                network_up,
    ...
    let (end_tx, end_rx) = bounded::<()>(1);
    {
        ...
        thread::spawn(move || {
            ...
            info!("DeviceAgent shutting down...");
    ...
    Ok(DeviceAgentController { shutdown: ctrl_tx, end_rx, data_tx })
}
...
const DEFAULT_CONFIG: &str = r#"
    ...
    [stream_metrics]
    enabled = false
    bridge_topic = "v1/p/{project_id}/d/{device_id}/telemetry/device_agent_stream_metrics"
    serializer_topic = "v1/p/{project_id}/d/{device_id}/telemetry/device_agent_serializer_stream_metrics"
    blacklist = []
    timeout = 10

    [serializer_metrics]
    enabled = false
    topic = "v1/p/{project_id}/d/{device_id}/telemetry/device_agent_serializer_metrics"
    timeout = 10

    [mqtt_metrics]
    enabled = true
    topic = "v1/p/{project_id}/d/{device_id}/telemetry/device_agent_mqtt_metrics"

    [streams.action_status]
    topic = "v1/p/{project_id}/d/{device_id}/commands/status"
    batch_size = 1
    flush_period = 2
...
fn parse_config(device_json: &str, config_toml: &str) -> Result<(Config, DeviceConfig), Error> {
    ...
    let replace_topic_placeholders = |topic: &mut String| {
        *topic = topic.replace("{project_id}", project_id).replace("{device_id}", device_id);
    };
        ...
        if stream_config.topic.is_empty() {
            stream_config.topic = match stream_config.compression {
                Compression::Disabled => {
                    format!("v1/p/{{project_id}}/d/{{device_id}}/telemetry/{stream_name}")
                }
                Compression::Lz4 => {
                    format!("v1/p/{{project_id}}/d/{{device_id}}/telemetry/{stream_name}")
                }
    ...
    if config.system_stats.enabled {
        for stream_name in [
            ...
            let stream_config = StreamConfig {
                topic: format!("v1/p/{project_id}/d/{device_id}/telemetry/{stream_name}"),
    ...
    if let Some(batch_size) = config.logging.as_ref().and_then(|c| c.stream_size) {
        let stream_config =
            config.streams.entry("logs".to_string()).or_insert_with(|| StreamConfig {
                topic: format!("v1/p/{project_id}/d/{device_id}/telemetry/logs"),
                batch_size: 32,
                ..Default::default()
            });
...
mod tests {
    ...
    #[test]
    fn default_config_uses_excalibur_topics() {
        ...
        let topics = [
            ...
            config.streams["device_shadow"].topic.as_str(),
            config.streams["action_status"].topic.as_str(),
            config.streams["device_agent_system_stats"].topic.as_str(),
            config.stream_metrics.bridge_topic.as_str(),
            config.serializer_metrics.topic.as_str(),
            config.mqtt_metrics.topic.as_str(),
        ];

        for topic in topics {
            assert!(topic.starts_with("v1/p/project-a/d/device-1/"));
            assert!(!topic.contains("/tenants/"));
            assert!(!topic.contains("{project_id}"));
            assert!(!topic.contains("{device_id}"));
        }
        assert_eq!(config.streams["custom"].topic, "v1/p/project-a/d/device-1/telemetry/custom");
        assert_eq!(config.streams["device_shadow"].topic, "v1/p/project-a/d/device-1/shadow");
        assert_eq!(
