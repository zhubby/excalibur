...
#			4. Add device_agent and startup scripts to systemd
#			5. Reboot the system
...
## Step 4: Add device_agent and other scripts to systemd
# Add device_agent to systemd
...
#			device_agent is running as expected
...
# Copy device_agent binary and config.toml to /usr/local/share/bytebeam folder of next rootfs
if [ ! -f /mnt/next_root/usr/local/share/bytebeam/device_agent ]
then
	cp /usr/local/share/bytebeam/device_agent /mnt/next_root/usr/local/share/bytebeam/device_agent
fi
