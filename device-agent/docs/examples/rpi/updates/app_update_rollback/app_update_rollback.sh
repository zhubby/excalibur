...
##			6. Send the status(success/failure) to device_agent
...
then
	...
	then
		echo "is active"
		# Send status(success) to device_agent)
		...
		if [ "$APP" != "device_agent" & "$INSTALL_AB" = "yes" ]
		then
		...
		# Send status(failed) to device_agent)
