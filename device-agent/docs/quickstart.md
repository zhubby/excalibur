...
select stream `device_agent_system_stats` and field `used_memory` in the first panel and stream `device_agent_processor_stats` and field `usage` in the second one.
...
(device_shadow, device_agent_system_stats for RAM, device_agent_processor_stats for CPU usage). You can also create streams and push data to them manually using uplink. 
