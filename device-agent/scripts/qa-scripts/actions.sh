...
docker cp devices/actions.toml simulator:/usr/share/bytebeam/device_agent/devices/actions.toml

docker exec -it simulator device_agent -a /usr/share/bytebeam/device_agent/devices/device_$DEVICE_ID.json -c /usr/share/bytebeam/device_agent/devices/actions.toml -vv -m device_agent::base::bridge
...
# Stop device_agent when an action is still in execution and check persistence directory for current_action file
# docker exec -it simulator tree /var/tmp/persistence
# Restart device_agent and connect with example python to send respose to persisted action(restart_response action)
