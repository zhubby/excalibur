...
# docker cp devices/downloader.toml simulator:/usr/share/bytebeam/device_agent/devices/downloader.toml

# docker exec -it simulator device_agent -a /usr/share/bytebeam/device_agent/devices/device_$DEVICE_ID.json -c /usr/share/bytebeam/device_agent/devices/downloader.toml -vv -m device_agent::base::bridge -m device_agent::collector::downloader
...
# Restart device_agent with partition attached that is smaller than the file to be downloaded
...
docker exec -it simulator sv stop /etc/runit/device_agent
docker exec -it simulator /usr/share/bytebeam/device_agent/simulator.sh download_auth_config $DEVICE_ID

printf "$(cat << EOF
[downloader]
actions = [{ name = "ota.install" }, { name = "send_file" }, { name = "send_script" }]
EOF
)" > devices/downloader.toml
docker cp devices/downloader.toml simulator:/usr/share/bytebeam/device_agent/devices/downloader.toml
