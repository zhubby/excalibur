...
This is a runnable commercial-platform scaffold, not a production-ready hosted service yet. The API can run with `STORAGE_BACKEND=memory` for fast development or `STORAGE_BACKEND=timescale` backed by SQL repositories. SQL mode stores hashed access sessions, rotating refresh tokens, used refresh-token replay markers, and hashed API keys durably; memory mode remains process-local for development. MQTT topic authorization, telemetry decoding, shadow updates, command status parsing, device-agent auth JSON, CSR/dev provisioning, certificate revoke, and action payload validation are implemented as tested library/API code. The standalone `mqtt-ingest` binary now starts a local MQTT v4 listener with `rumqttd` and consumes Excalibur native publish topics into the store; production mTLS peer-certificate identity hooks and NATS buffering remain the next hardening step.
