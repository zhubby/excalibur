...
impl TLSAcceptor {
    ...
    pub async fn accept(
        &self,
        stream: TcpStream,
    ) -> Result<(Option<String>, Option<String>, Box<dyn N>), Error> {
        match self {
            #[cfg(feature = "use-rustls")]
            TLSAcceptor::Rustls { acceptor } => {
                ...
                #[cfg(feature = "verify-client-cert")]
                let (tenant_id, peer_certificate_fingerprint_sha256) = {
                    ...
                    let peer_certificate = peer_certificates
                        ...
                        .as_ref();
                    (
                        extract_tenant_id(peer_certificate)?,
                        Some(certificate_fingerprint_sha256(peer_certificate)),
                    )
                };
                #[cfg(not(feature = "verify-client-cert"))]
                let tenant_id: Option<String> = None;
                #[cfg(not(feature = "verify-client-cert"))]
                let peer_certificate_fingerprint_sha256: Option<String> = None;

                let network = Box::new(stream);
                Ok((tenant_id, peer_certificate_fingerprint_sha256, network))
            }
            #[cfg(feature = "use-native-tls")]
