...
impl<P: Protocol + Clone + Send + 'static> Server<P> {
    ...
    async fn tls_accept(
        &self,
        stream: TcpStream,
    ) -> Result<(Box<dyn N>, Option<String>, Option<String>), Error> {
        #[cfg(any(feature = "use-rustls", feature = "use-native-tls"))]
        match &self.config.tls {
            Some(c) => {
                let (tenant_id, peer_certificate_fingerprint_sha256, network) =
                    TLSAcceptor::new(c)?.accept(stream).await?;
                Ok((network, tenant_id, peer_certificate_fingerprint_sha256))
            }
            None => Ok((Box::new(stream), None, None)),
        }
        #[cfg(not(any(feature = "use-rustls", feature = "use-native-tls")))]
        Ok((Box::new(stream), None, None))
    }
    ...
    pub async fn start(&mut self, link_type: LinkType) -> Result<(), Error> {
        ...
        loop {
            ...
            let (network, tenant_id, peer_certificate_fingerprint_sha256) =
                match self.tls_accept(stream).await {
                Ok(o) => o,
                Err(e) => {
            ...
            info!(
                ...
                count,
                tenant=?tenant_id,
                peer_certificate_fingerprint_sha256=?peer_certificate_fingerprint_sha256,
                "accept"
            );
                ...
                LinkType::Websocket => {
                    ...
                    task::spawn(
                        remote(
                            config,
                            tenant_id.clone(),
                            peer_certificate_fingerprint_sha256.clone(),
                            router_tx,
                            stream,
                            protocol,
...
async fn remote<P: Protocol>(
    ...
    peer_certificate_fingerprint_sha256: Option<String>,
    router_tx: Sender<(ConnectionId, Event)>,
    stream: Box<dyn N>,
    protocol: P,
