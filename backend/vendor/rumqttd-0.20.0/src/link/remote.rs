...
pub enum Error {
    ...
    InvalidAuth,
    #[error("Publish topic is not UTF-8")]
    InvalidPublishTopic,
...
impl<P: Protocol> RemoteLink<P> {
    pub async fn new(
        ...
        let (link_tx, link_rx, notification) = LinkBuilder::new(client_id, router_tx)
            ...
            .publish_auth(publish_auth.clone())
            .subscribe_auth(subscribe_auth)
...
pub async fn mqtt_connect<P>(
    ...
    peer_certificate_fingerprint_sha256: PeerCertFingerprint,
) -> Result<Packet, Error>
where
    P: Protocol,
{
...
mod tests {
    ...
    #[tokio::test]
    async fn external_auth_receives_peer_certificate_fingerprint() {
        let login = login();
        let mut cfg = config();
        cfg.set_auth_handler_with_peer(|_, _, _, peer| async move {
            peer.as_deref() == Some("fingerprint")
        });
