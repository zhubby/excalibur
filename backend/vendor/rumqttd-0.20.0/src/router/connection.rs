...
use crate::Filter;
use crate::{protocol::LastWill, PublishAuthHandler, SubscribeAuthHandler, Topic};
...
pub struct Connection {
    ...
    /// Optional broker-specific authorization for subscription filters.
    pub subscribe_auth: Option<SubscribeAuthHandler>,
    /// Optional broker-specific authorization for inbound publish topics.
...
impl fmt::Debug for Connection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Connection")
            ...
            .field("last_will_properties", &self.last_will_properties)
            .field("events", &self.events)
            .field("topic_aliases", &self.topic_aliases)
            .field("broker_topic_aliases", &self.broker_topic_aliases)
