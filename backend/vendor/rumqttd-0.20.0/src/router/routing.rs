...
pub enum RouterError {
    ...
    #[error("Subscription not authorized for client {0} filter {1}")]
    SubscriptionNotAuthorized(String, Filter),
...
fn append_to_commitlog(
    ...
    let source_client_id = connection.client_id.clone();
    ...
    for filter_idx in filter_idxs {
        ...
        let publish_data =
            PublishData::from_client(source_client_id.clone(), publish.clone(), properties.clone());
...
fn forward_device_data(
    ...
    let forwards = publishes
        .into_iter()
        .map(|(publish_data, offset)| {
            let mut publish = publish_data.publish;
            let mut properties = publish_data.properties;
            ...
            Forward {
                cursor: offset,
                size: 0,
                source_client_id: publish_data.source_client_id,
                publish,
                properties,
            }
        });
...
fn validate_subscription(
    ...
    if let Some(subscribe_auth) = &connection.subscribe_auth {
        let authorized = subscribe_auth(connection.client_id.clone(), filter.path.clone());
        if !authorized {
            return Err(RouterError::SubscriptionNotAuthorized(
