...
pub type PeerCertFingerprint = Option<String>;
pub type AuthHandler = Arc<
    dyn Fn(ClientId, AuthUser, AuthPass, PeerCertFingerprint) -> Pin<Box<dyn std::future::Future<Output = bool> + Send>>
        + Send
        + Sync,
>;
...
impl ServerSettings {
    ...
    pub fn set_auth_handler_with_peer<F, O>(&mut self, auth_fn: F)
    where
        F: Fn(ClientId, AuthUser, AuthPass, PeerCertFingerprint) -> O + Send + Sync + 'static,
        O: IntoFuture<Output = bool> + 'static,
        O::IntoFuture: Send,
    {
        self.connections.set_auth_handler_with_peer(auth_fn)
    }

    pub fn set_subscribe_auth_handler<F>(&mut self, auth_fn: F)
    where
        F: Fn(ClientId, Filter) -> bool + Send + Sync + 'static,
    {
        self.connections.set_subscribe_auth_handler(auth_fn)
    }

    pub fn set_publish_auth_handler<F>(&mut self, auth_fn: F)
    where
        F: Fn(ClientId, Topic) -> bool + Send + Sync + 'static,
    {
...
impl ConnectionSettings {
    ...
    where
        ...
        O::IntoFuture: Send,
    {
        self.external_auth = Some(Arc::new(move |client_id, username, password, _peer| {
    ...
    where
        F: Fn(ClientId, Filter) -> bool + Send + Sync + 'static,
    {
        self.subscribe_auth = Some(Arc::new(auth_fn));
    }
